...
getNode             = return . T.getNode
{-# INLINE getNode #-}

-- | select the attribute of the root of a tree

hasNode :: (Tree t, MonadSeq m) => (b -> Bool) -> t b -> m (t b)
...
deep :: (Tree t, MonadSeq m) => (t b -> m c) -> (t b -> m c)
deep f
...
deepest :: (Tree t, MonadSeq m) => (t b -> m c) -> (t b -> m c)
deepest f
...
multi :: (Tree t, MonadSeq m) => (t b -> m c) -> (t b -> m c)
multi f
...
processBottomUpWhenNot :: (Tree t, MonadSeq m) =>
                          (t b -> m (t b)) -> (t b -> m (t b)) -> (t b -> m (t b))
...
processTopDownUntil :: (Tree t, MonadSeq m) =>
                       (t b -> m (t b)) -> (t b -> m (t b))
...
insertChildrenAt :: (Tree t, MonadSeq m) =>
...
insertChildrenAfter :: (Tree t, MonadSeq m) =>
                       (t b -> m (t b)) -> (t b -> m (t b)) -> (t b -> m (t b))
...
-- Example
--
-- > insertTreeTemplateTest :: MonadSeq m => b -> m XmlTree
...
insertTreeTemplate :: (Tree t, MonadSeq m) =>
