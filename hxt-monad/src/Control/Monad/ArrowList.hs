...
arrL :: MonadList m => (b -> [c]) -> b -> m c
arrL f = fromList . f

arr2L :: MonadList m => (b -> c -> [d]) -> (b, c) -> m d
arr2L = arrL . uncurry

constA :: Monad m => c -> b -> m c
constA = arr . const

constL :: MonadList m => [c] -> b -> m c
constL = arrL . const
...
(>>.) :: MonadList m => (b -> m c) -> ([c] -> [d]) -> (b -> m d)
f >>. g = \ x -> toList (f x) >>= return . g >>= fromList

(>.) :: MonadList m => (b -> m c) -> ([c] ->  d ) -> b -> m d
af >. f = af >>. ((:[]) . f)

listA :: MonadList m => (b -> m c) -> b -> m [c]
listA af = af >>.  (:[])

unlistA :: MonadList m => [b] -> m b
unlistA = arrL id
...
single :: MonadList m => (b -> m c) -> b -> m c
single f = f >>. take 1
...
perform :: MonadList m => (b -> m c) -> (b -> m b)
