...
arrL :: MonadList s m => (b -> [c]) -> b -> m c
arrL f = fromList . f

arr2L :: MonadList s m => (b -> c -> [d]) -> (b, c) -> m d
arr2L = arrL . uncurry

constA :: Monad m => c -> b -> m c
constA = arr . const

constL :: MonadList s m => [c] -> b -> m c
constL = arrL . const

isA :: MonadList s m => (b -> Bool) -> b -> m b
isA p x
    | p x       = return x
    | otherwise = returnS mzero

(>>.) :: MonadList s m => (b -> m c) -> ([c] -> [d]) -> (b -> m d)
f >>. g = \ x -> toList (f x) >>= return . g >>= fromList

(>.) :: MonadList s m => (b -> m c) -> ([c] ->  d ) -> b -> m d
af >. f = af >>. ((:[]) . f)

listA :: MonadList s m => (b -> m c) -> b -> m [c]
listA af = af >>.  (:[])

unlistA :: MonadList s m => [b] -> m b
unlistA = arrL id

this :: Monad m => b -> m b
this = arr id

none :: MonadList s m => b -> m c
none = zeroArrow
...
single :: MonadList s m => (b -> m c) -> b -> m c
single f = f >>. take 1
...
perform :: MonadList s m => (b -> m c) -> (b -> m b)
perform f = listA f &&& this >>> arr snd

catA :: (MonadPlus m, MonadList s m) => [b -> m c] -> b -> m c
catA = foldl (<+>) none
