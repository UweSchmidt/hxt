...
module Text.XML.HXT.Monad.Pickle.Xml
where
...
import           Text.XML.HXT.Monad.Edit          (xshowEscapeXml)
import           Text.XML.HXT.Monad.ReadDocument  (xread)
import           Text.XML.HXT.Monad.WriteDocument (writeDocumentToString)
import           Text.XML.HXT.Monad.XmlState

{- just for embedded test cases, prefix with -- to activate
import           Text.XML.HXT.Monad.XmlArrow
...
-- by a schema. A schema for a data type can be defined by 'Text.XML.HXT.Monad.Pickle.Schema.scDT'.
-- In 'Text.XML.HXT.Monad.Pickle.Schema' there are some more functions for creating
...
-- 'Text.XML.HXT.Monad.Edit.xshowEscapeXml' into a string,
-- this function will escape all XML special chars, such that pickling the value back becomes save.
-- Pickling is done with 'Text.XML.HXT.Monad.ReadDocument.xread'
